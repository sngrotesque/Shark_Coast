...
static wU32 pack4(const wByte *a)
{
    wU32 res = \
        static_cast<wU32>(a[0]) << 0 * 8 |
        static_cast<wU32>(a[1]) << 1 * 8 |
        static_cast<wU32>(a[2]) << 2 * 8 |
        static_cast<wU32>(a[3]) << 3 * 8;

    return res;
}

static void ChaCha20_block_next(const wU32 in[16], wU32 out[16], wByte **keystream)
{
    for(int i = 0; i < 16; i++) {
		out[i] = in[i];
	}
